...
class Index_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		$order_sql = 'select id,ordersn,createtime,address,price,invoicename from ' . tablename('ewei_shop_order') . ' where uniacid = :uniacid and deleted=0 AND ( status = 1 or (status=0 and paytype=3) ) ORDER BY createtime ASC LIMIT 20';
		$order = pdo_fetchall($order_sql, array(':uniacid' => $_W['uniacid']));
		$order_ok = array_map(function($value) {
