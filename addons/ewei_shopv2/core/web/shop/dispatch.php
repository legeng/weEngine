...
class Dispatch_EweiShopV2Page extends WebPage
{
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('ewei_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('ewei_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
