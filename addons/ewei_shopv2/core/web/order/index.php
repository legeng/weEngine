...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('ewei_shop_order') . ' where uniacid = :uniacid and status > 0 and createtime between :createtime1 and :createtime2';
		$param = array(':uniacid' => $_W['uniacid'], ':createtime1' => $createtime1, ':createtime2' => $createtime2);
		$pdo_res = pdo_fetchall($sql, $param);
		$res = array_map(function($arr) use(&$price) {
