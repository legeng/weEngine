	...
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($rows as $rownum => $col) {
				...
				$sql = 'select id,status,refundid from ' . tablename('ewei_shop_order') . ' where ordersn=:ordersn and uniacid=:uniacid';
				$sql .= ' and status in (1,2) and isverify=0 and isvirtual=0 and virtual=0 and addressid >0 limit 1';
				$order = pdo_fetch($sql, array(':ordersn' => $ordersn, ':uniacid' => $_W['uniacid']));
