...
if ($do == 'checkcoupon') {
	...
	$card_api = new Coupon();
	$result = $card_api->fetchCard($coupon['card_id']);
	if ($result['cash']['base_info']['status'] == 'CARD_STATUS_VERIFY_OK' || empty($coupon_id)) {
		...
		if (COUPON_TYPE == SYSTEM_COUPON) {
			...
			if (empty($id)) {
				...
				foreach ($post['coupons'] as $coupon) {
					$account = array('acid' => $_W['acid']);
					$card_id = pdo_getcolumn('coupon', array('uniacid' => $_W['uniacid'], 'id' => $coupon), 'card_id');
					$result = $weixin->sendCoupons($card_id, $openids);
					$post['members'] = serialize($post['members']);
					...
					if (is_error($result)) {
						...
						pdo_insert('coupon_activity', $post);
						message('发放任务提交成功。等待微信服务器发放卡券', url('activity/market'), 'success');
					}
				}
			}
		}
