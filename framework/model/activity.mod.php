...
function activity_coupon_info($id) {
	...
	if (!empty($modules)) {
		...
		foreach ($modules as $modulename => $row) {
			$coupon['modules'][] = array('title' => $unimodules[$modulename]['title'], 'name' => $modulename);
		}
	}
...
function activity_coupon_grant($id,$uid) {
	global $_W, $_GPC;
	if (empty($uid)) {
		$uid = $_W['member']['uid'];
	}
	...
	elseif (!empty($coupon['modules']) && !in_array($_W['current_module']['name'], $coupon['modules']) && $_GPC['c'] != 'activity') {
		return error(-1, '非法模块');
	}
	$give = $_W['activity_coupon_id'] ? true :false;
	$insert = array(
		'couponid' => $id,
		'uid' => trim($_W['member']['uid']),
		'uniacid' => $_W['uniacid'],
		'openid' => $fan['openid'],
		'code' => $code,
...
function activity_coupon_use($couponid, $recid, $module = 'system') {
	...
	if ($module == 'paycenter') {
		...
		if (!empty($coupon_info['modules']) && !in_array($module, $coupon['modules']) && $module != 'system') {
			return error(-1, '非法模块');
		}
	}
